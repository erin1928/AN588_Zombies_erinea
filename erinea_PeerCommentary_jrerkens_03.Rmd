...
Libraries, make them clear and easy to see! We should know what we need to download and what's going to be used throughout the rmd!
...
Make sure that your rmd can be replicated whenever someone pulls from github (or whatever)! Having things saved onto desktops are fine but is much easier to use curl when collaborating.

### 3

Add some more comments! Makes it a bit easier to see what's being calculated in each chunk.

### 4

I agree with you! sd() is so convenient, and you're probably never going to be calculating population sd or mean because if you had the entire population, there wouldn't be a need to be doing statistics/working w a sample.

### 5

The for loop situation you had going on is smart, and definitely the way to go for when replicate() doesn't work out for whatever reason.

## Overall good first draft! I like your use of color for ggplot2, and although I can't see plant vs. zombies image, I'm sure it's great <3.
...
knitr::opts_chunk$set(echo = TRUE) # commented this line so it didn't fail to compile
# zombie_data <- read.csv("~/Desktop/zombies.csv", sep = ",", header = TRUE, stringsAsFactors = FALSE)
zombie_data <- read_csv(curl("https://raw.githubusercontent.com/fuzzyatelin/fuzzyatelin.github.io/master/AN588_Fall23/zombies.csv"))
zombie_data
```

## Problem 1: Means
```{r read csv, include=TRUE}
knitr::opts_chunk$set(echo = TRUE) # you don't necessarily have to have this line in every chunk 
...
mean_height <- mean(height) # getting nitpicky but it's not really necessary to assign a variable to something as simple as mean
mean_height
...
sum_squared_diff_age <- sum((age - mean_age)^2) # this is annoying to do by hand, congrats!
...
box_plot <- box_plot + theme(axis.text.x = element_text(angle = 90)) #I personally prefer the text to be horizontal as opposed to vertical, element_text(angle =) arg
box_plot <- box_plot + ylab("Age") + xlab("Gender") # boxplot looks solid! 
box_plot
```

## Problem 2: Weight
```{r boxplot of weight, include=TRUE}
library(ggplot2) # you don't need to call to ggplot2 in every chunk <3
...
scatterplot <- scatterplot + geom_smooth(method = "lm") # nice linear model!
scatterplot
```
...
lower <- mean_height - qnorm(1 - 0.05/2) * height_sample_stand_error # 0.975 is the same value as 1 - 0.05/2
upper <- mean_height + qnorm(1 - 0.05/2) * height_sample_stand_error # can we assume normality here? i'd personally argue no but the point of the exercise is to know when and when not to use qnorm() vs. qt() in confidence interval calculations
...
lower_killed <- mean_killed - qnorm(1 - 0.05/2) * killed_sample_stand_error # we definitely can't assume normality here, would use qt()
...
height_99 <- NULL
for (i in 1:99) {
    height_99[i] <- mean(sample(height, 30, replace = FALSE)) # head's up this is only doing 99 iterations!
}
all_100 <- c(height_99, mean(sample_height))
mean(all_100)
sd(all_100) # double head's up i'm pretty sure we're supposed to be doing confidence intervals here
```
